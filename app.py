...
# "Cavabı göstər" sıxılıbsa, düzgün variantın sırasını tapıb CSS-i təhlükəsiz şəkildə hazırlayırıq
css_injection = ""
    ...
    if st.session_state.get("show_current_answer", False) and "exam_questions" in st.session_state:
        ...
        if 0 <= curr_idx < len(questions):
            ...
            if correct_text in options:
                ...
                # F-string daxilində xəta verməməsi üçün fiqurlu mötərizələri ikiqat ({{}}) etdik
                css_injection = f"""
...
# Əsas CSS Dizaynı (Sintaksis xətası verməməsi üçün tam təmizləndi)
st.markdown(f"""
    <style>
    .stApp {{
